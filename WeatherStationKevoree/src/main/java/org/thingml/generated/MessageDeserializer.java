...
final AtomicState state_MessageDeserializer_receive_Idle = new AtomicState("Idle")
{
@Override
public void onEntry() {
{
...
states_MessageDeserializer_receive.add(state_MessageDeserializer_receive_Idle);
...
transitions_MessageDeserializer_receive.add(new InternalTransition("Handler_433259692",receive_bytesType, in_port, state_MessageDeserializer_receive_Idle){
@Override
...
{
System.out.println("byte[] received from serial. Will try to decode it!...");
receive((byte[])ce.b);
}
}

});
final CompositeState state_MessageDeserializer_receive = new CompositeState("receive", states_MessageDeserializer_receive, state_MessageDeserializer_receive_Idle, transitions_MessageDeserializer_receive, regions_MessageDeserializer_receive, false){
@Override
public void onEntry() {
{
...
in_port = new Port(PortType.REQUIRED, "in", inEvents_in, outEvents_in, 1);
...
out_port = new Port(PortType.REQUIRED, "out", inEvents_out, outEvents_out, 2);
//Init state machine
...
send(temperatureType.instantiate(RemoteControl_port, (short) t__var), RemoteControl_port);
}
}
...
send(lightType.instantiate(RemoteControl_port, (short) t__var), RemoteControl_port);
}
}
...
short size__var = (short) (getPacketManager_DATA_POSITION__var() + ArrayDeserializer_receive_bytes__var[getPacketManager_LENGTH_POSITION__var() + 1]
 + 1);
